var tns = (function () {
  ...
  function addEvents(el, obj, preventScrolling) {
    for (var prop in obj) {
      var option =
        ["touchstart", "touchmove"].indexOf(prop) >= 0 && !preventScrolling ? passiveOption : false;
  ...
  var tns = function (options) {
    ...
    function getItemsMax() {
      ...
      if (autoWidth || (fixedWidth && !viewportMax)) {
        ...
        return Math.ceil(
          fixedWidth ? viewportMax / Math.min.apply(null, arr) : Math.max.apply(null, arr)
        );
      }
    }
    ...
    function initSheet() {
      ...
      if (CSSMQ) {
        ...
        if (TRANSITIONDURATION) {
          var str =
            middleWrapper && options.autoHeight ? getTransitionDurationStyle(options.speed) : "";
        ...
        if (carousel) {
          str =
            horizontal && !autoWidth
              ? "width:" +
                getContainerWidth(options.fixedWidth, options.gutter, options.items) +
                ";"
              : "";
          if (TRANSITIONDURATION) {
    ...
    function onNavClick(e) {
      ...
      if (hasAttr(target, "data-nav")) {
        var navIndex = (navClicked = Number(getAttr(target, "data-nav"))),
          targetIndexBase =
            fixedWidth || autoWidth ? (navIndex * slideCount) / pages : navIndex * items,
